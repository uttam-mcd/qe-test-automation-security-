...
public class ZapManager {
	...
	public int startZAP(String zapPath) throws Exception {
		if (process == null) {
			...
			params.add("-daemon");
			params.add("-host");
			params.add(ZapSecurityPrep.getZAP_PROXYHOST());// params.add( HOST);
			params.add("-port");
			params.add(String.valueOf(ZapSecurityPrep.getZAP_PROXYPORT()));// params.add(String.valueOf(port));
			params.add("-dir");
			params.add("tmp");
			params.add("-config");
			...
			log.info("Start ZAProxy [" + zapProgramFile.getAbsolutePath() + "] on port: "
					+ ZapSecurityPrep.getZAP_PROXYPORT());
	...
	public void stopZap() {
		...
		try {
			...
			ClientApi client = new ClientApi(ZapSecurityPrep.getZAP_PROXYHOST(), ZapSecurityPrep.getZAP_PROXYPORT());// new
																														// ClientApi(HOST,port,API_KEY);
			client.core.shutdown(ZapSecurityPrep.getZAP_APIKEY());
			Thread.sleep(2000);
			process.destroy();
	...
	private void waitForSuccessfulConnectionToZap() {
		...
		do {
			try {
				log.info("Attempting to connect to ZAP API on: " + ZapSecurityPrep.getZAP_PROXYHOST() + " port: "
						+ ZapSecurityPrep.getZAP_PROXYPORT());
				socket = new Socket();
				socket.connect(
						new InetSocketAddress(ZapSecurityPrep.getZAP_PROXYHOST(), ZapSecurityPrep.getZAP_PROXYPORT()),
